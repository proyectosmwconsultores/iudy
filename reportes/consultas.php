...
");
	...
	public function get_vacio_alumnos($IdGrupo,$IdCiclo){
		$db = new Conexion();
		$sql = $db->query("SELECT
			tblc_usuario.IdUsua,
tblc_usuario.Nombre,
...
WHERE
tblc_usuario.IdGrupo =  '$IdGrupo' AND tblc_usuario.IdEstatus = '8'
ORDER BY tblc_usuario.APaterno ASC, tblc_usuario.AMaterno ASC, tblc_usuario.Nombre ASC");
    while($x = $db->recorrer($sql)){
			$_c = $x['id_ciclo_reincorporacion'];
			if($IdCiclo <> $_c){
					$get_vacio_alumnos[] = $x;
...
tblp_calificacion._A,
tblp_calificacion._B,
tblp_calificacion._C
FROM
tblp_calificacion
WHERE
tblp_calificacion.IdUsua =  '$IdUsua' AND
tblp_calificacion.IdModulo =  '$IdModulo' AND
...
WHERE tblp_asignacion.IdCiclo = '$IdCiclo' AND tblp_asignacion.IdGrupo = '$IdGrupo' AND tblp_asignacion.Tipo = '2' ORDER BY tblp_modulo.CodeModulo ASC");
		...
		return $get_lstMateria;
	}

	public function get_vacio_lstMateria($IdOferta,$Grado){
		$db = new Conexion();

		$sql = $db->query("SELECT tblp_modulo.IdModulo,
tblp_modulo.CodeModulo,
tblp_modulo.NombreMod
FROM
tblp_modulo
WHERE tblp_modulo.IdEducativa = '$IdOferta' AND tblp_modulo.Grado = '$Grado'  ORDER BY tblp_modulo.CodeModulo ASC");
		while($x = $db->recorrer($sql)){
			$gmATRS[] = $x;
		}
		return $gmATRS;
