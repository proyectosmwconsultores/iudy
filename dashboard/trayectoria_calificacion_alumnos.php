    ...
    <table class="table table-striped" style="font-size: 12px;">
      <tbody>
        <tr>
          ...
          <?php $no = 0; for ($ma=0;$ma< sizeof($_matlst);$ma++) { $no = ($no + 1); ?>
          <th style="width: 10px; text-align: center;"><?php echo $_matlst[$ma]["CodeModulo"]; ?></th>
          <?php } ?>
          <th style="width: 10px; background: white; text-align: center;">PROM</th>
        </tr>
        ...
        <tr>
          ...
          <?php $_sum1 = 0; for ($m=0;$m< sizeof($_matlst);$m++) {
            $_total=$t->get_user_prom_tra($_user[$x]['IdUsua'],$_matlst[$m]['IdModulo'],$sql_lista_mat[$i]["IdCiclo"]); ?>

          <td style="text-align: center; "><?php if(isset($_total[0])){ echo $_total[0][0]; $_sum1 = ($_sum1 + $_total[0][0]); } ?></td>
          <?php   } ?>

          <td style="text-align: center; "><?php if(isset($_sum1)){
            echo $prom = ($_sum1 / $no);
          } ?></td>
        </tr><?php
...
<div class="bg-gray-active color-palette" style="padding: 8px;"><span style="color: black;"><i class="fa fa-fw fa-bookmark"></i> GRÁFICA DE TRAYECTORIA DE PROMEDIO</span></div>
...
<table id="datatable_us" class="table table-striped" style="font-size: 12px; ">
        <tbody>
          <tr>
            ...
            <?php for ($g=0;$g< sizeof($sql_p);$g++) { ?>
            <tr>
              <td><?php echo $sql_p[$g]['APaterno'].' '.$sql_p[$g]['AMaterno'].' '.$sql_p[$g]['Nombre']; ?></td>
              <td style="text-align: center;"><?php echo $sql_p[$g]['Promedio']; ?></td>
            </tr>
          <?php } ?>
        </tbody>
    </table>
    ...
    Highcharts.chart('container_us', {
        ...
        title: {
            text: 'GRÁFICA DE PROMEDIO DEL GRUPO'
        },
        yAxis: {
            allowDecimals: false,
            title: {
