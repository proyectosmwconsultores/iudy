<?php $section = "Reporte de alumnos"; include("head.php");
if($_SESSION['IdUsua']){ $addIngresos=$t->add_ingresos($_SESSION['IdUsua'],'Está visualizando el reporte de bajas'); }
...
<script>
	function cargar_lista_bajas(){
		...
		$(Capa).load("dashboard/alumnos_activos_periodo.php",{IdCampus:IdCampus, IdCiclo:IdCiclo}, function(response, status, xhr) {
			if (status == "error") {
	...
	function del_alumnoIdx(IdUsua,IdCiclo,IdCampus){
		$.ajax({
				 ...
				 method:"POST",
				 data:{IdUsua:IdUsua, IdCiclo:IdCiclo, IdCampus:IdCampus},
