<?php
require('php/clases/captura.php');
$cap = new captura();
$campus = $cap->get_campus();
...
<section id="wrapper" class="step-register">
  <div class="register-box">
    <div class="">
       <a onClick="window.open('./','_self')" href="javascript:void(0);" class="text-center db m-b-40"><img src="assets/images/campus/logo_inicio.png" style="width: 200px;"></a>
        <form id="msform" name="msform" action="registroAlumno.php" method="POST" enctype="multipart/form-data">
        ...
        <!-- fieldsets -->
        <fieldset id='btn1'>
        <h2 class="fs-title">¡Inscríbete ahora!</h2>
        <h3 class="fs-subtitle">Necesita registrarte con los siguientes datos:</h3>
        <select class="form-control" name="txtCampus" id="txtCampus" onchange="buscarOferta()">
        ...
        <select class="form-control" name="txtOferta" id="txtOferta">
          <option value="">- Seleccione oferta educativa- </option>
        </select><br>
        <input type="text" name="txtEmail" onchange="valEmail()" id="txtEmail" placeholder="Email" />

          <input onClick="window.open('continuar.php?x=1','_self')" href="javascript:void(0);" type="button" name="previous" class="previous action-button" value="Tengo código" />
          <input onClick="window.open('./','_self')" href="javascript:void(0);" type="button" name="previous" class="previous action-button" value="Regresar" />
...
<script>
  ...
  function buscarOferta(){
    ...
    var TipoGuardar = "searhOferta";
    $.post("formConsulta/setting.php", {TipoGuardar:TipoGuardar, IdCampus:IdCampus}, function(data){
      $("#txtOferta").html(data);
  ...
  function valCodex(){
    ...
    var TipoGuardar = "validar_code";
    $.ajax({
         url:"formConsulta/setting.php",
         method:"POST",
         data:{TipoGuardar:TipoGuardar, Code:Code},
         success:function(data){
    ...
    .done(function(data) {
      ...
      if(data==2){
        ...
        swal("Error de código", "El código ingresado no es válido, favor de comunicarse con su docente.", "error");
  ...
  // else {
  //   swal("Error de código", "El código deben ser de 10 caracteres, incluidos números y letras.", "error");
  //     document.getElementById("txtCode").focus();
  //     return 0;
  //
  // }
...
<script src="dist/login/register-init_alumno.js"></script>
