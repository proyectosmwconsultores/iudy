...
if (isset($_POST["Mov"]) && $_POST["Mov"] == "sub_Excelx") {
	$t->add_excel_conciliar_pagx();
	exit;
}
?>
		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="box box-default">
					<div class="box-body">
						<div class="row">
							<form name="frm" id="frm" action="conciliar_pagos.php" method="POST" enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<div class="input-group">
											...
											<span class="input-group-btn">
												<button type="button" class="btn bg-navy btn-flat" onClick="upload_excel()" style="float: right; margin-right: 5px;"><i class="fa fa-cloud-upload"></i> Subir excel</button>
											</span>
										</div>
									</div>
								</div>
...
<script>
	...
	function vista_lista_pagos() {
		...
		$(Capa).load("vistas/finanzas/proceso_validacion.php", {}, function(response, status, xhr) {
			if (status == "error") {
		...
		swal({
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					document.frm.Mov.value = 'sub_Excelx';
					document.frm.submit();
					return true;
				} else {
					return false;
	...
	function proceso_cobrar_pag(IdTemporal, IdUsua) {
		var Indicador = 1;
		$.ajax({
