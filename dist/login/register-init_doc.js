...
$(".submit").click(function(){
	...
	swal("Error al guardar", "Debe escribir su apellido materno.", "error");
		...
		swal({
			title: "\u00BFEst\u00E1 seguro que desea crear su cuenta para iniciar su proceso de inscripción?",
			type: "warning",
		...
		function (isConfirm) {
			if (isConfirm) {
				...
				$.ajax({
		         ...
		         method:"POST",
		         data:{TipoGuardar:TipoGuardar, Tipo:Tipo, Email:Email, Nombre:Nombre, Paterno:Paterno, Materno:Materno, Telefono:Telefono, Sexo:Sexo},
				...
				.done(function(data) {
					if(data==1){
						swal("Error al registrarse", "Usted ya esta dado de alta en la Plataforma, le recomendamos recuperar su cuenta.", "error");
					}

					if(data==2){
						swal({
			    		title: "Su registro ha sido exitoso, en un momento uno de nuestros asesores se pondrá en contacto contigo.",
			    		type: "success",
			    		confirmButtonColor: '#DD6B55',
			    		confirmButtonText: 'Aceptar',
			    	},
			    	function (isConfirm) {
			    		if (isConfirm) {
			    			parent.location.href='registro.php';
			    		}
			    	});
