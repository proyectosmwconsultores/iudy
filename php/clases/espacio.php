...
ORDER BY
  ...
  public function add_asigModulo(){
    ...
    $sql1e = $db->query("INSERT INTO tblp_tareas (IdAsignacion, IdAlumno, Visto, IdActividadesDocente, IdParcialDocente) VALUES ('$IdAsignacion','$IdUsua','1','$IdActividad','$IdParcial')");
    $IdTx = $db->insert_id;
    if($IdTipoA == 1){
...
WHERE tblp_pagos.IdUsua = '$IdUsua'  AND tblp_pagos.IdEstatus = '1'
...
ORDER BY tblc_ciclo.FInicio DESC
 ");
  ...
  public function get_infoPerfil($IdUsua) {
    ...
    $get_infoPerfil = [];
    $sql = $db->query("SELECT tblc_usuario.IdOferta, tblc_usuario.Nombre AS Alumno, tblc_usuario.SemCua, tblc_usuario.Semblanza,tblc_usuario.IdUsua, tblc_usuario.IdGrupo, tblc_usuario.Visto, tblc_usuario.Folio, tblc_usuario.FecLimite, tblc_campus.Campus, tblp_educativa.IdGrado, tblp_educativa.Nombre AS Oferta FROM tblc_usuario Left Join tblc_campus ON tblc_campus.IdCampus = tblc_usuario.IdCampus Left Join tblp_educativa ON tblp_educativa.IdEducativa = tblc_usuario.IdOferta WHERE tblc_usuario.IdUsua = '$IdUsua' ");
    while($x = $db->recorrer($sql)){
      $get_infoPerfil[] = $x;
    }
  ...
  public function get_chkAvisos($IdGrupoNew) {
    $db = new Conexion();

    #SE GENERA LA EVALUACIÓN DOCENTE PARA LOS COORDINADORES

    $sql_asig = $db->query("SELECT tblp_asignacion.IdAsignacion, tblp_asignacion.IdUsua, tblp_asignacion.IdCiclo, tblp_asignacion.IdCampus, tblp_asignacion.IdEducativa, tblp_asignacion.IdGrupo, tblp_asignacion.FecIni, tblp_asignacion.FecFin, tblp_asignacion.IdModulo FROM tblp_asignacion WHERE tblp_asignacion.Estatus =  'Activo' AND tblp_asignacion.Tipo =  '4' AND tblp_asignacion.Curso =  '0' LIMIT 50 ");
    while($s = $db->recorrer($sql_asig)){
      $sql8 = $db->query("SELECT tblp_asignacion.IdUsua FROM tblp_asignacion WHERE tblp_asignacion.Tipo = '2' AND tblp_asignacion.IdAsignacion = '".$s['IdAsignacion']."' LIMIT 1");
      $db->rows($sql8);
      $datos81 = $db->recorrer($sql8);
      $IdDocente = $datos81["IdUsua"];
      
      $insertar = $db->query("INSERT INTO tblx_evaluacion (IdCiclo, IdCampus, IdOferta, IdGrupo, IdUsua, FecCap, IdEstatus, FecIni, FecFin, IdTipo, IdModulo, IdAsignacion, Tipo) VALUES ('".$s['IdCiclo']."','".$s['IdCampus']."','".$s['IdEducativa']."','".$s['IdGrupo']."','".$s['IdUsua']."',NOW(),'31','".$s['FecIni']."','".$s['FecFin']."','1','".$s['IdModulo']."','".$s['IdAsignacion']."','4')");
      $Id_eva = $db->insert_id;
      $sql2 = $db->query("SELECT tblx_pregunta.IdPregunta FROM tblx_pregunta WHERE tblx_pregunta.Tipo =  '1' AND tblx_pregunta.Permisos = '4' AND tblx_pregunta.IdEstatus =  '8'");
      while($y = $db->recorrer($sql2)){
        $IdP = $y["IdPregunta"];
          $insertar = $db->query("INSERT INTO tblx_respuesta (IdPregunta,IdDocente,IdEvaluacion,IdGrupo,IdUsua, FecCap,IdOferta,IdCampus,IdAsignacion,IdEstatus,IdModulo, IdCiclo)  VALUES('$IdP','$IdDocente','$Id_eva','".$s['IdGrupo']."','".$s['IdUsua']."',NOW(),'".$s['IdEducativa']."','".$s['IdCampus']."','".$s['IdAsignacion']."','8','".$s['IdModulo']."','".$s['IdCiclo']."')");
      }
      $insertar = $db->query("UPDATE tblp_asignacion SET tblp_asignacion.Curso = 1 WHERE tblp_asignacion.IdAsignacion = '".$s['IdAsignacion']."' ");
    }

    #SE ACTIVA LA EVALUACION PARA LOS USUARIOS SEGUN LA FECHA
    $hoy = date("Y-m-d");
    $sql_eva = $db->query("SELECT tblx_evaluacion.IdEvaluacionX FROM tblx_evaluacion WHERE tblx_evaluacion.IdEstatus = 31 AND tblx_evaluacion.FecIni <= '$hoy' ");
    while($e = $db->recorrer($sql_eva)){
      $insertar = $db->query("UPDATE tblx_evaluacion SET tblx_evaluacion.IdEstatus = 8 WHERE tblx_evaluacion.IdEvaluacionX = '".$e['IdEvaluacionX']."' ");
    }


    $gAvisr = [];
    $sql = $db->query("SELECT tblc_avisodetalle.IdAvisoD, tblc_avisodetalle.IdAviso, tblc_avisodetalle.IdGrupo, tblc_aviso.Titulo, tblc_aviso.Aviso, tblc_aviso.Archivo, tblc_aviso.FecCap, tblc_aviso.Tipo FROM tblc_avisodetalle Left Join tblc_aviso ON tblc_aviso.IdAviso = tblc_avisodetalle.IdAviso WHERE tblc_avisodetalle.IdGrupo = '$IdGrupoNew' ORDER BY tblc_aviso.FecCap DESC LIMIT 4");
    while($x = $db->recorrer($sql)){
      $gAvisr[] = $x;
    }
    return $gAvisr;
...
WHERE tblc_aviso.Permisos = '2' AND tblc_avisoasesor.IdCampus = '$IdCampus' ORDER BY tblc_aviso.FecCap DESC LIMIT 4 ");
  ...
  public function get_pagPend($IdUsua) {
    $db = new Conexion();
    $get_pagPend = [];
    $sql = $db->query("SELECT tblp_pagos.Fecha, tblp_pagos.FecDesc FROM tblp_pagos WHERE tblp_pagos.IdUsua =  '$IdUsua' AND tblp_pagos.IdEstatus <>  '4' AND tblp_pagos.IdConceptoPlan <> '12' ORDER BY tblp_pagos.FecDesc ASC");
    while($x = $db->recorrer($sql)){
      $get_pagPend[] = $x;
    }
    return $get_pagPend;
  ...
  public function get_chk_pagos_pendientes($IdUsua) {
    $db = new Conexion();
    $hoy = date("Y-m-d");
    $sql9 = $db->query("SELECT tblc_usuario.Folio FROM tblc_usuario WHERE tblc_usuario.IdUsua = '$IdUsua'");
    $db->rows($sql9);
    $datos91 = $db->recorrer($sql9);
    $_folio = $datos91['Folio'];
    ...
    tblp_pagos.Fecha <  '$hoy'
    ");
    while($x = $db->recorrer($sql)){
      $get_pag_pen_user[] = $x;
    }
    return $get_pag_pen_user;
    }
  }
  ...
  public function get_chkEncuenta($IdUsua) {
    ...
    // $db->rows($sql8);
    // $datos81 = $db->recorrer($sql8);
    // $Id_evax = isset($datos81['IdEvaluacionX']);
    // if($Id_evax){
...
Left Join tblc_tipoevaluacion ON tblc_tipoevaluacion.IdTipoEvaluacion = tblx_evaluacion.IdTipo WHERE tblx_evaluacion.IdUsua =  '$IdUsua' ORDER BY tblx_evaluacion.IdCiclo ASC");
  ...
  public function get_chk_docs_pen($IdUsua,$IdOferta) {
    $db = new Conexion();
    $get_chk_docs_pen = [];

      $sql9 = $db->query("SELECT tblp_educativa.IdGrado FROM tblp_educativa WHERE tblp_educativa.IdEducativa =  '$IdOferta'");
      $db->rows($sql9);
      $datos91 = $db->recorrer($sql9);
      $IdGrado = $datos91['IdGrado'];
      $sql = $db->query("SELECT * FROM tblh_tipodocumento WHERE tblh_tipodocumento.Grado = '$IdGrado' AND tblh_tipodocumento.Solicitado = '1' ");
      while($x = $db->recorrer($sql)){
        $sql_docs = $db->query("SELECT * FROM tblc_docalumnos WHERE tblc_docalumnos.IdTipoDocumento = '".$x['IdTipoDoc']."' AND tblc_docalumnos.IdUsua =  '$IdUsua'");
        $db->rows($sql_docs);
        $datos_docs = $db->recorrer($sql_docs);
        if(!isset($datos_docs)){
          $get_chk_docs_pen[] = $x;
        }
      }
...
Left Join tblh_tipodocumento ON tblh_tipodocumento.IdTipoDoc = tblc_docalumnos.IdTipoDocumento WHERE tblc_docalumnos.IdUsua = '$IdAlumno' AND tblc_docalumnos.Estatus <> 4 AND tblc_docalumnos.IdTipoDocumento < 100 ");
  ...
  public function get_mis_avisos_id($IdUsua) {
    ...
    FROM
    tbla_aviso_detalle
    Left Join tbla_aviso ON tbla_aviso.IdAviso = tbla_aviso_detalle.IdAviso
    WHERE
  ...
  public function get_proceso_inscripcion_id($IdUsua) { 
    $db = new Conexion();
    $get_proceso_inscripcion_id = [];
    $sql = $db->query("SELECT tblc_alumnos.IdActivo, tblc_alumnos.Valor FROM tblc_alumnos WHERE tblc_alumnos.IdUsua =  '$IdUsua' ORDER BY tblc_alumnos.Valor DESC LIMIT 1");
    while($x = $db->recorrer($sql)){
      $get_proceso_inscripcion_id[] = $x;
    }
    return $get_proceso_inscripcion_id;
  }

  public function get_avisos_publico() { 
    $db = new Conexion();
    $get_avisos_publico = [];
    $sql = $db->query("SELECT * FROM tbla_aviso WHERE tbla_aviso.Valor = 1");
    while($x = $db->recorrer($sql)){
      $get_avisos_publico[] = $x;
    }
    return $get_avisos_publico;
  }
  ...
  public function get_mis_avisos_practica_id($IdUsua) {
        ...
        while($x = $db->recorrer($sql)){
          ...
          if(!isset($_sql4['IdPractica'])){
            ...
            if(!isset($_sql11['IdPractica'])){
              //$sql_fac = $db->query("SELECT * FROM tblp_practicas WHERE tblp_practicas.IdUsua = '$IdUsua' AND tblp_practicas.IdAviso = '$IdAviso' AND tblp_practicas.IdEstatus = '1'");
              //echo "SELECT * FROM tblp_practicas WHERE tblp_practicas.IdUsua = '$IdUsua' AND tblp_practicas.IdAviso = '$IdAviso' AND ((tblp_practicas.IdEstatus = '1') || (tblp_practicas.IdEstatus = '2') || (tblp_practicas.IdEstatus = '3') || (tblp_practicas.IdEstatus = '5'))";
    ...
    // $sql_gpx = $db->query("SELECT tblc_ciclogrupo.Grado FROM tblc_ciclogrupo WHERE tblc_ciclogrupo.IdGrupo = '$_IdGrupo' AND tblc_ciclogrupo.Grado >=  '4'");
    // $db->rows($sql_gpx);
    // $_goxs = $db->recorrer($sql_gpx);
    // $_grapxy = isset($_goxs['Grado']);
    ...
    //   if(($Grad == 3) && ($_grapx == 5)){
    //     $sql9 = $db->query("SELECT tblc_usuario.IdUsua, tblc_usuario.IdGrupo, tblp_educativa.IdGrado FROM tblc_usuario Left Join tblp_educativa ON tblp_educativa.IdEducativa = tblc_usuario.IdOferta WHERE tblc_usuario.IdUsua =  '$IdUsua'");
    //     $db->rows($sql9);
    //     $datos91 = $db->recorrer($sql9);
    //     $Grad = $datos91['IdGrado'];
    ...
    //       // $sql_fac = $db->query("SELECT * FROM tblp_practicas WHERE tblp_practicas.IdUsua = '$IdUsua' AND tblp_practicas.IdAviso = '$IdAviso' AND tblp_practicas.IdEstatus = '4'");
    //       // $db->rows($sql_fac);
    //       // $_fac = $db->recorrer($sql_fac);
    //       // if(!isset($_fac['IdPractica'])){
    //       //   $sql_fac = $db->query("SELECT * FROM tblp_practicas WHERE tblp_practicas.IdUsua = '$IdUsua' AND tblp_practicas.IdAviso = '$IdAviso' AND tblp_practicas.IdEstatus = '11'");
    //       //   $db->rows($sql_fac);
    //       //   $_fac = $db->recorrer($sql_fac);
    //       //   if(!isset($_fac['IdPractica'])){
    //       //     $sql_ins = $db->query("INSERT INTO tblp_practicas (IdUsua, IdEstatus, IdAviso) VALUES ('$IdUsua',1,'$IdAviso') ");
    ...
    //     }
    //     // echo "SELECT * FROM tblp_practicas WHERE tblp_practicas.IdUsua = '$IdUsua' AND tblp_practicas.IdEstatus = '4'";
    //   }
    //   // if(!$IdPractica){
    //     return $get_mis_avisos_practica_id;
    //   // }
  ...
  public function get_mis_avisos_servicio_id($IdUsua) {
    $db = new Conexion();
    ...
    $sql9 = $db->query("SELECT tblc_usuario.IdUsua, tblc_usuario.porcentaje, tblc_usuario.IdGrupo, tblc_usuario._horario, tblp_educativa.IdGrado FROM tblc_usuario Left Join tblp_educativa ON tblp_educativa.IdEducativa = tblc_usuario.IdOferta WHERE tblc_usuario.IdUsua =  '$IdUsua'");
    $db->rows($sql9);
    $datos91 = $db->recorrer($sql9);
    $Grad = $datos91['IdGrado'];
    $_IdGrupo = $datos91['IdGrupo'];
    $_porcentaje = $datos91['porcentaje'];

    $get_mis_avisos_practica_id = [];
    if(($Grad == 3) && ($_porcentaje >= 68)){
      $IdAviso = 0;
      ...
      $sql_prac = $db->query("SELECT * FROM tblp_servicio WHERE tblp_servicio.IdUsua = '$IdUsua' AND tblp_servicio.IdAviso = '$IdAviso' AND tblp_servicio.IdEstatus = '4'");
      $db->rows($sql_prac);
      $_prac = $db->recorrer($sql_prac);

      if (isset($_prac['IdServicio'])) {
        return [];
      }
      return $get_mis_avisos_practica_id;
    }
...
");
  ...
  public function get_servicio_social_pendiente($IdUsua) {
    $db = new Conexion();

    
    $sql9 = $db->query("SELECT tblc_usuario.IdUsua, tblp_educativa.IdGrado FROM tblc_usuario Left Join tblp_educativa ON tblp_educativa.IdEducativa = tblc_usuario.IdOferta WHERE tblc_usuario.IdUsua =  '$IdUsua'");
    $db->rows($sql9);
    $datos91 = $db->recorrer($sql9);

    
    if($datos91['IdGrado']== 3){
    $get_servicio_social_pendiente = [];
    ...
    WHERE tblp_servicio.IdUsua =  '$IdUsua' AND tblp_servicio.IdEstatus =  '31' AND tbla_aviso_servicio.IdEstatus =  '8' ");
        while($x = $db->recorrer($sql)){
          $get_servicio_social_pendiente[] = $x;
        }
        return $get_servicio_social_pendiente;
    }
  ...
  public function get_total_creditos($IdOferta,$IdCampus,$Termino) {
		$db = new Conexion();
