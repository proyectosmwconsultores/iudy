<?php
	...
	$beca=$t->get_beca($_SESSION["IdUsua"],substr($_GET["tokenId"],10,10));
	$boucher=$t->get_boucherId(substr($_GET["tokenId"],10,10));
	$bancos=$t->get_bancos($boucher[0]["IdOferta"]);
	$Nobancos=$t->get_nobancos($boucher[0]["IdOferta"]);
	$recargo=$t->get_recargo($_SESSION["IdUsua"],substr($_GET["tokenId"],10,10));
  $FecIns=date("Y-m-d");
	$valor = 0;
	$msj= 0;
...
<page backtop="10mm" backbottom="20mm" backleft="10mm" backright="10mm">
	...
	<table>
		...
		<tr style="background: #fff9f9;">
			...
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: left; color: #0b73b5;"><?php echo $boucher[0]["Nombre"].' '.$boucher[0]["APaterno"].' '.$boucher[0]["AMaterno"]; ?></td>
		</tr>
		<tr style="background: #fff9f9;">
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: right; color: #0b73b5;"><b>CONCEPTO:</b></td>
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: left; color: #0b73b5;"><?php echo $boucher[0]["NomPlan"]; ?></td>
		</tr>
		<tr style="background: #fff9f9;">
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: right; color: #0b73b5;"><b>ESTATUS:</b></td>
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: left; color: #0b73b5;"><?php echo $boucher[0]["Estatus"]; ?></td>
		</tr>
		<tr style="background: #fff9f9;">
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: right; color: #0b73b5;"><b>TOTAL CONCEPTO:</b></td>
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: left; color: #0b73b5;">$ <?php echo number_format($boucher[0]["Monto"], 2, '.', ','); ?></td>
		</tr>
		<?php if($boucher[0]["Descuento"]){ ?>
		<tr style="background: #fff9f9;">
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: right; color: #0b73b5;"><b>DESCUENTO:</b></td>
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: left; color: #0b73b5;">$ <?php echo number_format($boucher[0]["Descuento"], 2, '.', ','); ?></td>
		</tr><?php } ?>
		<?php if($recargo[0]["Recargo"]){ ?>
		<tr style="background: #fff9f9;">
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: right; color: #0b73b5;"><b>RECARGO:</b></td>
			<td colspan="2" style="width: 280px; font-size: 12px; border: none; text-align: left; color: #0b73b5;">$ <?php echo number_format($recargo[0]["Recargo"], 2, '.', ','); ?></td>
		</tr><?php } ?>
		<tr style="background: whitesmoke;">
			<td colspan="4" style="width: 560px; font-size: 12px; border: none; text-align: center; "><b style="font-size: 25px; color: #066db0;">
			Total pagar: $ <?php echo number_format($totPagar, 2, '.', ','); ?></b></td>
		</tr>
