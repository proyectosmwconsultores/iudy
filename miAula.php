...
$IdAsignacion = isset($_GET['idAsignacion']) ? $_GET['idAsignacion'] : '';
...
<body class="hold-transition skin-<?php echo $configuracion[15]["Descripcion"]; ?> sidebar-mini fixed">
    ...
    <div class="content-wrapper">
      <div class="topbar-planeacion">
        <div class="topbar-planeacion__left">
          <h1> <i class="fa fa-flag"></i> MI PLANEACIÓN </h1>
        </div>
          ...
          <div class="partials-wrap">
            <div class="partials">
              <?php if ($totalParciales == 3) { ?>
                <button onClick="window.open('miAula.php?idAsignacion=<?php echo urlencode($IdAsignacion); ?>&idToks=<?php echo $parciales[0]['IdParcialDocente']; ?>','_self')" class="partial p1">
                  <span>Parcial 1</span>
                </button>
                <button onClick="window.open('miAula.php?idAsignacion=<?php echo urlencode($IdAsignacion); ?>&idToks=<?php echo $parciales[1]['IdParcialDocente']; ?>','_self')" class="partial p2">
                  <span>Parcial 2</span>
                </button>
                <button onClick="window.open('miAula.php?idAsignacion=<?php echo urlencode($IdAsignacion); ?>&idToks=<?php echo $parciales[2]['IdParcialDocente']; ?>','_self')" class="partial p3">
                  <span>Parcial 3</span>
                </button>
              <?php } ?>
              <?php if ($totalParciales == 2) { ?>
                ...
                <button onClick="window.open('miAula.php?idAsignacion=<?php echo urlencode($IdAsignacion); ?>&idToks=<?php echo $parciales[1]['IdParcialDocente']; ?>','_self')" class="partial p3">
                  <span>Parcial 2</span>
                </button>
                </button>
              <?php } ?>
                  ...
                  <?php foreach ($semana['actividades'] as $actividad) { ?>
                    <div class="item item-actividad" data-idactividad="<?php echo $actividad['IdActividadesDocente']; ?>" style="cursor:pointer;">
                      <div>
                        ...
                        <div class="tag activity" <?php if ($actividad['IdEstatus'] == 8) {
                                                    echo "style='color: #9696ff;'";
                                                  } else {
                                                    echo "style='color: #ff4f4f;'";
                                                  } ?>>
  ...
  <script>
    ...
    function mi_chat_id(employee_id) {
      $.ajax({
    ...
    function iniciarExamenEspecial(IdAsignacion, IdParcial, IdSemana, IdActividad, IdTarea) {

      Swal.fire({
        title: '¿Está seguro que desea iniciar esta evaluación?',
        text: 'Recuerde que una vez iniciado comenzará a correr su tiempo.',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          window.location.href = 'mi_examen.php?idAsignacion=' +
            IdAsignacion + '&idLeccion=' +
            IdParcial + '_' + IdSemana + '_' + IdActividad + '_' + IdTarea;
        }
      });
