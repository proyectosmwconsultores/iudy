...
if($_SESSION['IdUsua']){ $addIngresos=$t->add_ingresos($_SESSION['IdUsua'],'Está visualizando el reporte de cuenta de bancos'); }
...
$pagoId=$t->get_reportPagosB($_POST["txtBanco"],$_POST["txtOferta"],$_POST["txtConcepto"],$_POST["txtCicloEscolar"],$_POST["txtGrupo"]);
?>
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			<section class="content-header">
				<h1>
					Reporte de cuenta de bancos
				</h1>
				<ol class="breadcrumb">
					<li><a href="#"><i class="fa fa-book"></i> Reportes</a></li>
					<li class="active">Cuenta de banco</li>
				</ol>
			</section>
			<section class="content">
				<form name="frm" id="frm" action="repBancosA.php" method="POST" enctype="multipart/form-data">
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<div class="input-group">
										...
										<select class="form-control" name="txtBanco" id="txtBanco" onchange="document.frm.submit();">
											<option value=""> - Seleccione - </option>
											<?php for ($i=0;$i< sizeof($bancos);$i++) { ?>
											<option value="<?php echo $bancos[$i]["IdBanco"]; ?>"<?php if($_POST["txtBanco"]==$bancos[$i]["IdBanco"]){?>selected="selected"<?php }?>><?php echo $bancos[$i]["Banco"]; ?></option>
											<?php } ?>
										</select>
									</div>
								</div>
											...
											<table id="example" class="table table-striped table-bordered table-hover dataTables-example" >
												...
												<tbody>
													<?php for ($i=0;$i< sizeof($pagoId);$i++) {
														$suma = $suma + $pagoId[$i]["TotalPagado"];

														  ?>
													<tr>
														<td><?php echo $pagoId[$i]["NomEducativa"]; ?></td>
														...
														<td><?php echo $pagoId[$i]["Ciclo"]; ?></td>
														<td><?php echo $pagoId[$i]["NomConcepto"]; ?></td>
														<td style="text-align: right;">$
															<?php echo number_format($pagoId[$i]["TotalPagado"], 2, '.', ',');  ?>
														</td>
												...
												<tfoot>
                <tr><th rowspan="1" colspan="8" style="text-align: right;">Total:</th><th rowspan="1" colspan="1" style="text-align: right;">$ <?php echo number_format($suma, 2, '.', ',');  ?></th></tr>
                </tfoot>
											</table>
