<?php
  ...
  $sql_mod74 = $db->query("SELECT tblc_modulousuario.IdModUsua FROM tblc_modulousuario WHERE tblc_modulousuario.IdUsua = '$IdUsua' AND tblc_modulousuario.IdModulo = '77'");
  $db->rows($sql_mod74);
  $mod74 = $db->recorrer($sql_mod74);
  ...
  <div class="box-body">
    ...
    <table id="example" class="table table-striped table-bordered table-hover dataTables-example" style="font-size: 12px;" >
      ...
      <tbody>
        ...
        <tr>
          <td>
            <button title="Eliminar asignación" type="button" class="btn bg-maroon btn-flat btn-sm view_del" href="javascript:void(0);" name="view" value="view" id="<?php echo $matx["IdAsignacion"]; ?>" style="float: center;"><i class="fa fa-trash"></i></button>
            <button type="button" title="Editar asignación" class="btn bg-navy btn-flat btn-sm view_editar" href="javascript:void(0);" name="view" value="view" id="<?php echo $matx["IdAsignacion"]; ?>" style="float: center;"><i class="fa fa-edit"></i></button>
            <button type="button" title="Captura de horario de clases" class="btn bg-purple btn-flat btn-sm view_horario" href="javascript:void(0);" name="view" value="view" id="<?php echo $matx["IdAsignacion"]; ?>" style="float: center;"><i class="fa fa-clock-o"></i></button>
            <!-- <button type="button" title="Evaluación docente" class="btn bg-orange btn-flat btn-sm view_evaluacion" href="javascript:void(0);" name="view" value="view" id="<?php echo $matx["IdAsignacion"]; ?>" style="float: center;"><i class="fa fa-file-o"></i></button> -->
            <?php if(isset($mod74['IdModUsua'])){ ?>
            <button type="button" title="Copiar planeación académica" class="btn bg-olive btn-flat btn-sm view_copiar" href="javascript:void(0);" name="view" value="view" id="<?php echo $matx["IdAsignacion"].'_'.$matx["IdModulo"].'_'.$matx["IdCiclo"]; ?>" style="float: center;"><i class="fa fa-copy"></i></button>
            <?php } ?>
            <button type="button" title="Lista de alumnos en la materia" class="btn bg-yellow btn-flat btn-sm view_users" href="javascript:void(0);" name="view" value="view" id="<?php echo $matx["IdAsignacion"]; ?>" style="float: center;"><i class="fa fa-users"></i></button>
          </td>
