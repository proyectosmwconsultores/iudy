...
<?php
  ...
  $params = array(
      ...
      "user"=>"iudyopenpay@gmail.com",
      "password"=> "x19pedro25x"
        );
  ...
  if($tipoGuardar == "process_fact_id"){
    ...
    $_diax = substr($Fecha, 8, 2);
    
    $Fecha = $_aniox.'-'.$_mesx.'-'.$_diax;
    $_fechax = date('H:i:s');
    $_fecha = $Fecha."T".$_fechax;
    ...
    if($_user_dat['IdCampus'] == 6){ $Serie = "O"; }
    
    if(!$Serie){
        $_procesado = "0_Favor de comunicarse con el administrador no existe la serie de la factura.";
        echo $_procesado;
        die();
    }
    ...
    $_idfolio_ok = $_folio_dis['IdFolio'];
    if(!$_folio_ok){
...
WHERE tblp_foliospago.IdUsua = '$IdUsua' AND  tblp_foliospago.NoFolio = '$NoFolio' ");
    ...
    while($_pag = $db->recorrer($sql_pag)){
      ...
      if($fac_us['IdGrado'] == 4){
          $_complemento = 1;
          $_rfc = $_user['RFC'];
          $_alumno = $fac_us['Nombre'].' '.$fac_us['APaterno'].' '.$fac_us['AMaterno'];
          $_curp = $fac_us['Curp'];
          $_rvoe = $fac_us['Rvoe'];
          $_nivel = 'Bachillerato o su equivalente';
            ...
            "Any" => [
                [
                    "iedu:instEducativas" => [
                        ...
                        "@version" => "1.0",
                        "@rfcPago" => "$_rfc", // RFC del pagador
                        "@nombreAlumno" => "$_alumno", // Reemplaza con nombre dinámico si aplica
                        "@CURP" => "$_curp", // Reemplaza si tienes este dato
                        "@nivelEducativo" => "Bachillerato o su equivalente", // Cambia según el grado: Preescolar, Primaria, Secundaria, etc.
                        "@autRVOE" => "$_rvoe" // Número del RVOE oficial
                    ]
                ]
            ]
        ];
      ...
      $insertar = $db->query("INSERT INTO tblg_conceptos_factura (FolioPago,ClaveProdServ,Cantidad,ClaveUnidad,Unidad,Descripcion,ValorUnitario,Importe,Descuento,ObjetoImp, FecCap, Precio, _folio, _codigoFactura, _unidad, complemento, rvoe, curp, alumno, nivel) VALUES('$NoFolio','".$_pag['ClaveProdServ']."','1','".$_pag['ClaveUnidad']."','$unidad','$nomP','$va_unitario','$va_importe','$va_descuento','01', NOW(),'$va_unitario','$_folio','$codigoFactura','$_unidad','$_complemento','$_rvoe','$_curp','$_alumno','$_nivel')");
      $i = ($i + 1);
    ...
    try {
        ...
        if($res == 'success'){
                ...
                $noCertificadoCFDI = $resultadoJson->data->noCertificadoCFDI;
                ...
                */
                ...
                $rutaFactura = "../../assets/docs/factura/$_anio/$_mes/";

                if (!is_dir($rutaFactura)) {
                    mkdir($rutaFactura, 0775, true);
                }
                ...
                */
                ...
                $imgdata = base64_decode($qrCode, true);

                if ($imgdata === false || empty($imgdata)) {
                    die("Error: el código QR no es un base64 válido.");
                }

                $path = $rutaFactura . $nombre_img . ".png";

                if (file_put_contents($path, $imgdata) === false) {
                    die("Error: no se pudo guardar la imagen QR.");
                }
                ...
                */
                ...
                $nombrex = $_folio . '.xml';
                $rutaXml = $rutaFactura . $nombrex;

                if (empty($cfdi)) {
                    die("Error: el CFDI XML viene vacío.");
                }

                $archivo = fopen($rutaXml, "w");

                if (!$archivo) {
                    die("Error al crear el archivo XML.");
                }
  ...
  if($tipoGuardar == "generar_factura_global"){
    ...
    $_fechax = "19:10:05";
    $_fecha = $Fecha."T".$_fechax;
    //$_fecha = "2023-02-28T23:10:05";



    $sql_form = $db->query("SELECT * FROM tblc_formapago WHERE tblc_formapago.IdFormaPago =  '$IdForma'");
    $db->rows($sql_form);
    $_form = $db->recorrer($sql_form);
    $Forma = $_form['c_FormaPago'];
  ...
  $codigoFactura = $_anio.$codigoFactura.$_mes;
    ...
    $_folio = $codigoFactura;
    
    $e_rfc = "CIE090115D22"; 
    $e_nombre = "CENTRO INTEGRAL DE ESTUDIOS PROFESIONALES"; //"ESCUELA KEMPER URGATE";
    $e_regimen = "601";
    $e_cp = "86280";
    ...
    $sql_dat_us = $db->query("SELECT
tblp_foliospago.IdFolio,
tblp_foliospago.IdCiclo,
tblp_foliospago._descuento,
tblp_foliospago._total
FROM
tblp_foliospago
WHERE tblp_foliospago.IdForma = '$IdForma' AND tblp_foliospago.Factura = '1' AND tblp_foliospago.FecPago BETWEEN '$Inicio' AND '$Final' LIMIT 1
 ");
    $db->rows($sql_dat_us);
    $_user_dat = $db->recorrer($sql_dat_us);
    ...
    $informacionGlobal["Periodicidad"]="04";
    $informacionGlobal["Meses"]="$_mesx";
    $informacionGlobal["Año"]="$_aniox";
    ...
    $receptor["DomicilioFiscalReceptor"] = "86280";
...
tblp_foliospago.FecPago BETWEEN '$Inicio' AND '$Final' ");
    ...
    while($_pag = $db->recorrer($sql_pag)){
        ...
        $va_descuento = $_pag['_descuento'];
        if($va_descuento < 0){
          $va_unitario = $_pag['_total'];
          $va_descuento = 0;
          $va_importe = $_pag['_total'];
        } else {
          $va_unitario = $_pag['_importe'];
          $va_importe = $_pag['_total'];
          $_sum = ($va_descuento + $va_importe);
          if($_sum <> $va_unitario){
            $va_unitario = $_pag['_total'];
            $va_descuento = 0;
            $va_importe = $_pag['_total'];
        ...
        $Folio = "VENTA".$_pag['NoFolio'];
        $NoFolio = $_pag['NoFolio'];
        $_IdFolio = $_pag['IdFolio'];
      ...
      $concepto["ClaveProdServ"] = "01010101"; //$_pag['ClaveProdServ'];
      $concepto["NoIdentificacion"] = "$Folio";
      $concepto["Cantidad"] = "1";
      $concepto["ClaveUnidad"] = "ACT"; //$_pag['ClaveUnidad'];
      $concepto["Unidad"] = "ACTIVIDAD"; //$unidad;
      $concepto["Descripcion"] = "VENTA"; //$nomP;
      $concepto["ValorUnitario"] = $va_unitario;
      $concepto["Importe"] = $va_unitario;
      $concepto["Descuento"] = $va_descuento;
      $concepto["ObjetoImp"] = "02";
      ...
      $insertar = $db->query("UPDATE tblp_foliospago SET tblp_foliospago._codigoFactura = '$codigoFactura' WHERE tblp_foliospago.IdFolio = '$_IdFolio' ");
      
      $insertar = $db->query("INSERT INTO tblg_conceptos_factura (FolioPago,ClaveProdServ,Cantidad,ClaveUnidad,Unidad,Descripcion,ValorUnitario,Importe,Descuento,ObjetoImp, FecCap, Precio, _folio, _codigoFactura) VALUES('$NoFolio','01010101','1','ACT','ACTIVIDAD','$Folio','$va_unitario','$va_importe','$va_descuento','01', NOW(),'$va_unitario','$_folio','$codigoFactura')");
      $i = ($i + 1);
      $_sumImp= ($_sumImp + $va_unitario);
      $_sumDes= ($_sumDes + $va_descuento);
    }
    ...
    $insertar = $db->query("INSERT INTO tblg_datos_factura (FolioPago,IdEstatus, Version, FormaPago, Serie, Folio, Fecha, MetodoPago, Sello, NoCertificado, Certificado, CondicionesDePago, SubTotal, Descuento, Moneda, TipoCambio, Total, TipoDeComprobante, Exportacion, LugarExpedicion, E_Rfc, E_Nombre, E_RegimenFiscal, R_Rfc, R_Nombre, R_DomicilioFiscalReceptor, R_RegimenFiscalReceptor, R_UsoCFDI, Anio, Mes, Json, FecCap, NoSerie_emisor, _folio, _codigoFactura, _tipo) VALUES('$NoFolio','12','4.0','$Forma','V','$_folio_ok','$_fecha','PUE','---','-----','******','CondicionesDePago','$_sumImp','$_sumDes','MXN','1','$_tox','I','01','$e_cp','$e_rfc','$e_nombre','$e_regimen','XAXX010101000','PUBLICO EN GENERAL','86280','616','S01','$_anio','$_mes','$nombre',NOW(),'$NoSerie_emisor','$_folio','$codigoFactura','G')");
    $IdGenerar = $db->insert_id;
    ...
    if($cadenaOriginalSAT){
      ...
      $insertar = $db->query("INSERT INTO tblg_factura (FolioPago, IdGenerar, IdEstatus, cadenaOriginalSAT, noCertificadoSAT, noCertificadoCFDI, uuid, selloSAT, selloCFDI, fechaTimbrado, qrCode, cfdi, Fecha, IdUsua, _folio, _Grado, _Grupo, _idCiclo, _codigoFactura, _tipo) VALUES('$NoFolio','$IdGenerar','4','$cadenaOriginalSAT','$noCertificadoSAT','$noCertificadoCFDI','$uuid','$selloSAT','$selloCFDI','$fechaTimbrado', '$qrCode','$cfdi','$hoy','2','$_folio','1','A', '".$_user_dat['IdCiclo']."','$codigoFactura','G')");
      $IdFac = $db->insert_id;
      $insertar = $db->query("UPDATE tblp_foliospago SET tblp_foliospago.IdFactura = '$IdFac', tblp_foliospago.Factura = '3' WHERE tblp_foliospago._codigoFactura = '$codigoFactura' ");
      $insertar = $db->query("UPDATE tblg_conceptos_factura SET tblg_conceptos_factura.IdFactura = '$IdFac' WHERE tblg_conceptos_factura.FolioPago = '$NoFolio' ");
      $insertar = $db->query("UPDATE tblg_datos_factura SET tblg_datos_factura.IdFactura = '$IdFac' WHERE tblg_datos_factura.FolioPago = '$NoFolio' ");
      $insertar = $db->query("UPDATE tblg_folio SET tblg_folio.IdEstatus = '6', tblg_folio.FecUtilizado = NOW(), tblg_folio._codeFactura = '$codigoFactura' WHERE tblg_folio.IdFolio = '$_idfolio_ok' ");
      $_procesado = "1_1";
      ...
      $insertar = $db->query("DELETE FROM tblg_datos_factura WHERE tblg_datos_factura.FolioPago = '$NoFolio' ");
      $insertar = $db->query("UPDATE tblg_folio SET tblg_folio.IdEstatus = '8' WHERE tblg_folio.IdFolio = '$_idfolio_ok' ");
      //$insertar = $db->query("UPDATE tblp_foliospago SET tblp_foliospago.Factura = '1' WHERE tblp_foliospago._folio = '$_folio' ");
      $insertar = $db->query("DELETE FROM tblg_factura WHERE tblg_factura._codigoFactura = '$codigoFactura' ");
      $_procesado = "0_".$message;
    }
  ...
  if($tipoGuardar == "cancelar_factura_id"){
    $uuid = $_POST["uuid"];
    $motivo = $_POST["Motivo"];
    $_procesadox = 0;
    ...
    $_uuid = $result->data->uuid;
   echo $status = $result->status;

    if($status == 'success'){
        $sql_dat_us = $db->query("SELECT tblg_factura._codigoFactura FROM tblg_factura WHERE tblg_factura.uuid =  '$uuid' ");
        $db->rows($sql_dat_us);
        $_user_dat = $db->recorrer($sql_dat_us);
        $_codigoFactura = $_user_dat['_codigoFactura'];
    
        $insertar = $db->query("UPDATE tblg_factura SET tblg_factura.IdEstatus = '7' WHERE tblg_factura.uuid = '$uuid' ");
        $insertar = $db->query("UPDATE tblp_foliospago SET tblp_foliospago.Factura = '1' WHERE tblp_foliospago._codigoFactura = '$_codigoFactura' ");
        $insertar = $db->query("UPDATE tblg_datos_factura SET tblg_datos_factura.IdEstatus = '7' WHERE tblg_datos_factura._codigoFactura = '$_codigoFactura' ");
        $insertar = $db->query("UPDATE tblg_folio SET tblg_folio.IdEstatus = '7' WHERE tblg_folio._codeFactura = '$_codigoFactura' ");
        $_procesadox = 1;
    }
  ...
  function limpiar_cadena_id(string $cadena){
        $cadena=trim($cadena);
