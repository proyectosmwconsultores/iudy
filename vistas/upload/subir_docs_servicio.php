...
if (is_array($_FILES) && count($_FILES) > 0) {
  ...
  if ($archivo) {
    ...
    if (move_uploaded_file($_FILES["file"]["tmp_name"], "../../assets/docs/files/$anio/$mes/$archivo")) {
      ...
      $insertar = $db->query("INSERT INTO tblp_servicio_docs (IdUsua, IdServicio, IdTipo, Ruta, IdEstatus, FecCap, Formato) VALUES ('$IdUsua','$IdServicio','$Tipo','$ruta',2,NOW(),'$tipox') ");
      $db->close();
      echo $insertar;
      exit();
    }
