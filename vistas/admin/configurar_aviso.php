...
WHERE tblc_ciclogrupo.IdCiclo = '$IdCiclo'
GROUP BY tblp_grupo.IdOferta,
tblp_grupo.IdCampus
ORDER BY
      ...
      <tr>
        <td>
          <?php while ($_sel = $db->recorrer($sql_sel)) {
            $grpx = $db->query("SELECT tbla_aviso_detalle.IdDetalle FROM tbla_aviso_detalle WHERE tbla_aviso_detalle.IdAviso = '$IdAviso' AND tbla_aviso_detalle.IdGrupo = '".$_sel['IdGrupo']."'");
            $db->rows($grpx);
