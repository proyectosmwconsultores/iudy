...
FROM
tblp_libro
WHERE tblp_libro.IdOferta = '$IdOferta' AND tblp_libro.Grado = '$Grado' AND tblp_libro.IdTema = '9' ORDER BY tblp_libro.Code ASC");


  ?>
  <div class="bg-red-active color-palette" style="padding: 10px;"><span><?php echo $Grado; ?>° <?php echo $txtM; ?></span> - <span>Mis libros de consultas UDS</span></div>
  <ul class="mailbox-attachments clearfix">
    <?php while($x = $db->recorrer($sql)){
      $lib = '../assets/images/modulo/'.$x["IdGrado"].'/'.$x["IdTema"].'/'.$x["Oferta"].$x["Code"].'.png';
    ...
    <li onclick="mostrarLib(<?php echo$x["IdLibro"]; ?>)" href="javascript:void(0);" style="cursor: pointer;">
      <span class="mailbox-attachment-icon has-img"><img style="height: 180px; " src="assets/images/modulo/<?php echo $x["IdGrado"]; ?>/<?php echo $x["IdTema"]; ?>/<?php echo $img; ?>" alt="Attachment"></span>
    </li>
    <?php } ?>
  </ul>
