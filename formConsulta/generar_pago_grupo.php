...
if(isset($_POST["IdCiclo"])){
  ...
  $sql_con = $db->query("SELECT tblc_conceptosdetalle.IdConceptoDetalle, tblc_conceptosdetalle.IdConceptoPlan, tblc_conceptosdetalle.IdOferta, tblc_conceptosdetalle.IdConcepto, tblc_conceptosplanes.NomPlan, tblc_conceptosplanes.Costo FROM tblc_conceptosdetalle Left Join tblc_conceptosplanes ON tblc_conceptosplanes.IdConceptoPlanes = tblc_conceptosdetalle.IdConceptoPlan WHERE tblc_conceptosdetalle.IdOferta =  '$idOferta' AND tblc_conceptosplanes.IdConcepto =  '2'");
...
tblp_pagos.IdConceptoPlan =  '$IdPlan' AND
tblp_pagos.IdModulo =  '$IdModulo'
ORDER BY
  ...
  <form class="form-horizontal" name="frmx" id="frmx" action="addSubPeriodo.php" method="POST" enctype="multipart/form-data">
    <div class="box-body">
      <div class="form-group">
        ...
        <div class="col-sm-8">
          <select name="txtPlan" id="txtPlan" class="form-control" onchange="cambiar_plan(<?php echo $IdModulo.','.$IdCiclo.','.$IdGrupo; ?>)">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-4">
          <input type="text" name="txt_fecha" id="txt_fecha" class="form-control" value="">
        </div>
      </div>
      </div>
...
<div class="box-footer" style="text-align: right;">
  <?php if($vx){ ?>
    <button onclick="upd_generar_pg_id(<?php echo $IdGrupo.','.$IdAdmin.','.$IdCiclo.','.$IdModulo.','.$IdCal; ?>)" type="button" class="btn bg-maroon btn-flat margin"><i class="fa fa-refresh"></i> Actualizar pago</button>
  <?php } else { ?>
    <button onclick="generar_pg_id(<?php echo $IdGrupo.','.$IdAdmin.','.$IdCiclo.','.$IdModulo; ?>)" type="button" class="btn bg-purple btn-flat margin"><i class="fa fa-refresh"></i> Generar pago</button>
  <?php } ?>
</div>
<?php } ?>
...
<script>
  ...
  function generar_pg_id(IdGrupo, IdAdmin, IdCiclo, IdModulo){
      var TipoGuardar = "gen_pago_materia";
      ...
      if(!Fecha){
    	...
    	swal({
    		title: "\u00BFEst\u00E1 seguro que desea generar el pago de esta materia?",
    		type: "warning",
    	...
    	function (isConfirm) {
          ...
          $.ajax({
      	       ...
      	       method:"POST",
      	       data:{TipoGuardar:TipoGuardar, IdGrupo:IdGrupo, IdModulo:IdModulo, IdPlan:IdPlan, IdAdmin:IdAdmin, IdCiclo:IdCiclo, Fecha:Fecha},
              ...
              //load_user_beca();
              $.ajax({
          	       url:"formConsulta/generar_pago_grupo.php",
          	       method:"POST",
          	       data:{IdModulo:IdModulo, IdCiclo:IdCiclo, IdGrupo:IdGrupo, IdPlan:IdPlan},
          	       success:function(data){
  ...
  function upd_generar_pg_id(IdGrupo, IdAdmin, IdCiclo, IdModulo, IdCalendario){
      var TipoGuardar = "upd_gen_pago_materia";
      var IdPlan = document.getElementById("txtPlan").value;
      var Fecha = document.getElementById("txt_fecha").value;
      if(!IdPlan){
  			swal("Error al guardar", "Debe seleccionar el plan de pago.", "error");
      ...
      if(!Fecha){
    	...
    	swal({
    		title: "\u00BFEst\u00E1 seguro que desea actualizar la fecha del pago de esta materia?",
    		type: "warning",
    		showCancelButton: true,
    		confirmButtonColor: '#DD6B55',
    		confirmButtonText: 'Aceptar',
  ...
  function cambiar_plan(IdModulo, IdCiclo, IdGrupo){
    var IdPlan = document.getElementById("txtPlan").value;
	  $.ajax({
	       url:"formConsulta/generar_pago_grupo.php",
	       method:"POST",
