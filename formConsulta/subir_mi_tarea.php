...
$sql8 = $db->query("SELECT tblp_tareas.IdTarea, tblp_tareas.FecFinal, tblp_tareas.Link, tblp_tareas.Link2, tblp_tareas.Link3 FROM tblp_tareas WHERE tblp_tareas.IdAsignacion = '".$_POST["IdAsignacion"]."' AND tblp_tareas.IdActividadesDocente = '".$_POST["IdActividad"]."' AND tblp_tareas.IdAlumno= '$IdUsua'");
$db->rows($sql8);
...
$esp = 1;
if($datos_act['IdEstatus'] == 8){
  $idEstatus = 8;
} else {
  $idEstatus = 26;
}

if(isset($datos81['FecFinal'])){
  $fecFinal = $datos81['FecFinal'];
  $hoy = date("Y-m-d");
  if($fecFinal >= $hoy){
    $esp = 2;
    $idEstatus = 8;
...
$sqlmate = $db->query("SELECT tblp_asignacion.IdAsignacion, tblp_modulo.NombreMod FROM tblp_asignacion Left Join tblp_modulo ON tblp_modulo.IdModulo = tblp_asignacion.IdModulo WHERE tblp_asignacion.IdAsignacion =  '".$_POST["IdAsignacion"]."' AND tblp_asignacion.Tipo =  '2'");
$db->rows($sqlmate);
$datos_mat = $db->recorrer($sqlmate);
$Pagina = "Esta en la materia -> ".$datos_mat["NombreMod"];
$actv = "Subir tarea -> actividad -> ".$datos_act['NomActividad'];


$insertar = $db->query("INSERT INTO tblh_ingresos (IdUsua, Pagina, FecCap, _accion, _modulo, _valor, _idUsua, _idActividad)  VALUES ('$IdUsua', '$Pagina',NOW(),'Subir tarea','$actv','".$_POST["IdAsignacion"]."','$IdUsua','".$_POST["IdActividad"]."')");
  ...
  <div class="principal">
    <form action="" id="form_subir" class="form-horizontal">
        ...
        <div class="col-sm-4">
          <div class="checkbox">
            <label>
              ...
              <?php if($datos81["Link"]) { $ar1 = $datos81["Link"];  echo "<b style= 'color: red'>(1) Archivo activo</b>"; } else { echo "<b>(1) Espacio libre</b>"; } ?>
            </label>
        ...
        <div class="col-sm-4">
          <div class="checkbox">
            <label>
              ...
              <?php if($datos81["Link2"]) { echo "<b style= 'color: red'>(2) Archivo activo</b>"; } else { echo "<b>(2) Espacio libre</b>"; } ?>
            </label>
          </div><br>
        ...
        <div class="col-sm-4">
          <div class="checkbox">
            <label>
              ...
              <?php if($datos81["Link3"]) { echo "<b style= 'color: red'>(3) Archivo activo</b>"; } else { echo "<b>(3) Espacio libre</b>"; } ?>
            </label>
          </div><br>
      ...
      <div class="box-footer">
        <button name="bntSubir" id="bntSubir" type="button"  onclick="val_cargar_mitarea()" class="btn btn-success pull-right"><i class="fa fa-fw fa-save"></i> Cargar archivo <?php if($esp == 2){ echo '(Tiempo especial hasta el: '.$fecFinal.')'; } ?></button>
      </div><?php } ?>
      <?php if($idEstatus == 26){ ?>
