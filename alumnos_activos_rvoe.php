...
//$sql_8 = $db->query("SELECT tblc_alumnos.IdActivo, Count(tblc_usuario._idRvoe) AS Total FROM tblc_alumnos Left Join tblc_usuario ON tblc_usuario.IdUsua = tblc_alumnos.IdUsua WHERE tblc_alumnos.IdCiclo =  '$IdCiclo' AND tblc_usuario.IdCampus =  '$IdCampus' AND tblc_usuario._idRvoe =  '".$mat['IdRvoe']."' AND tblc_usuario.IdEstatus =  '8' GROUP BY tblc_usuario._idRvoe ");
  ...
  <div class="box-body">
    ...
    <table class="table table-striped" style="font-size: 12px;">
      <tbody>
        <tr>
          <th colspan="5" style="background: #aaafff;">REPORTE DE ALUMNOS REPROBADOS</th>
        </tr>
        <tr>
          <th></th>
          <th>CAMPUS RVOE</th>
      ...
      <tr>
        ...
        <td style="text-align: center; cursor: pointer;" onclick="mostrar_lista_user_reprobados(<?php echo $IdCampus.','.$IdCiclo.','.$rep[$n]['IdRvoe']; ?>)"><?php echo $rep[$n]['Total']; ?></td>
      </tr>
      <?php $s = ($s + $rep[$n]['Total']); } ?>
      <tr>
          <td colspan="4" style="text-align: right;"><b>TOTAL:</b></td>
          <td style="text-align: center;"><b><?php echo $s; ?></b></td>
      </tr>
    </tbody></table>
  </div>
