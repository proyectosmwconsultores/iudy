<?php
  ...
  if($tipo == 1) {
    ...
    $carpeta = "assets/docs/Videos/"; //nombre de la carpeta en la que se guardaran los archivos (si es en el directorio ponga /)
    ...
    $tipo = $nombreImg[1];
    $archivo = md5(rand() * time()).$archivo; // Generamos un nombre de archivo Aleatorio para evitar conflictos entre los nombres.
    ...
    $db = new Conexion();
    $sql9 = $db->query("SELECT Link FROM tblp_videos WHERE tblp_videos.IdVideo = '$IdVideo'");
    $db->rows($sql9);
    ...
    $insertar = $db->query("UPDATE tblp_videos SET tblp_videos.Link = '$archivo', tblp_videos.Tipo = '$tipo', tblp_videos.Estatus = '8' WHERE tblp_videos.IdVideo = '$IdVideo' ");
    if ($insertar) {
      $_SESSION['Alerta']="1";
      echo "<script type='text/javascript'>window.location='doSelDatosM.php';</script>";
    } else {
    ...
    $db = new Conexion();
    $sql1 = $db->query("SELECT $NoArchivo, IdTarea FROM tblp_tareas WHERE tblp_tareas.IdAsignacion ='$idAsignacion' AND tblp_tareas.IdActividadesDocente = '$IdActividadDoc' AND tblp_tareas.IdAlumno= '$IdUsua'");
    $db->rows($sql1);
    ...
    $nombreImg[1]; // Extención de la imagen
    $tipo = $nombreImg[1];
    $archivo = md5(rand() * time()).'.'.$tipo; // Generamos un nombre de archivo Aleatorio para evitar conflictos entre los nombres.
    if(!move_uploaded_file($_FILES["archivo"]['tmp_name'], $carpeta.$archivo)){
      $_SESSION['Alerta']="0";
      echo "<script type='text/javascript'>window.location='miAsignatura.php';</script>";
      exit();
    }
    ...
    if ((file_exists($nombre_fichero)) && ($folio)) {
      $insertar = $db->query("UPDATE tblp_tareas SET $NoArchivo = '$archivo', FecCap = NOW(), $Fec = NOW()  WHERE tblp_tareas.IdAsignacion ='$idAsignacion' AND tblp_tareas.IdActividadesDocente = '$IdActividadDoc' AND tblp_tareas.IdAlumno= '$IdUsua'");
      ...
      $sql3 = $db->query("SELECT Correo, Nombre, APaterno, AMaterno FROM tblc_usuario WHERE tblc_usuario.IdUsua = '$IdUsua'");
      $db->rows($sql3);
      $datos31 = $db->recorrer($sql3);
        ...
        <table border='0' cellpadding='0' width='90%' style='border-collapse: collapse; font-family:Century Gothic,Arial; '>
          ...
          <td colspan='3'>
            <table border='1' cellpadding='0' width='100%' style='border-collapse: collapse; font-family:Century Gothic,Arial; '>
              ...
              <tr>
                <td style=' text-align: right; padding: 10px;'>  Folio: </td><td style='padding: 10px;'> <b>  ";  $cuerpo .= htmlentities($folio)." </b></td>
              </tr>
            </table>
          </td>
          <tr>
      ...
      if($insertar) {
        ...
        echo "<script type='text/javascript'>window.location='alMiParcial.php';</script>";
      } else {
    ...
    $IdActividadDoc = $_POST["IdActividadDoc"];
    $idAsignacion = $_POST["IdAsignacion"];
    $Descripion = $_POST["txtImgDes"];

    // $NoArchivo = $_POST["NoArchivo"];
    if($_POST['chkLink11']) { $NoArchivo = "Link"; $com = "Comen1"; $Fec = "Fec1"; }
    if($_POST['chkLink22']) { $NoArchivo = "Link2"; $com = "Comen2"; $Fec = "Fec2"; }
    if($_POST['chkLink33']) { $NoArchivo = "Link3";  $com = "Comen3"; $Fec = "Fec3"; }
    ...
    $archivo = $_FILES["archivo"]['name']; //nombre del archivo
    $tamaño = $_FILES["archivo"]['size']; //tamaño del archivo
    $tipoo = $_FILES["archivo"]['type']; //tamaño del archivo
    $nombreImg = explode(".", $archivo);
    $nombreImg[1]; // Extención de la imagen
    $tipo = $nombreImg[1];
    $archivo = md5(rand() * time()).'.'.$tipo; // Generamos un nombre de archivo Aleatorio para evitar conflictos entre los nombres.
    ...
    if (file_exists($nombre_fichero)) {
      $insertar = $db->query("UPDATE tblp_tareas SET $NoArchivo = '$archivo', $com = '$Descripion', FecCap = NOW(), $Fec = NOW() WHERE tblp_tareas.IdAsignacion ='$idAsignacion' AND tblp_tareas.IdActividadesDocente = '$IdActividadDoc' AND tblp_tareas.IdAlumno= '$IdUsua'");

      $sql2 = $db->query("SELECT * FROM tblp_actividadesdocente WHERE tblp_actividadesdocente.IdAsignacion = '$idAsignacion' AND tblp_actividad.IdActividadesDocente = '$IdActividadDoc'");
      $db->rows($sql2);
      $datos21 = $db->recorrer($sql2);
      $nomActividad = $datos21["NomActividad"];
      ...
      <body>
        <table border='0' cellpadding='0' width='90%' style='border-collapse: collapse; font-family:Century Gothic,Arial; '>
          ...
          <tr>
              <td colspan='3' height='100' align='center'>
              <b style='color:#0b283e; font-size:24px; text-align: center; font-family:Century Gothic,Arial;'>
              Confirmaci&oacute;n de $nomActividad respondida
              </b></td>
          </tr>
          <tr>
          <td colspan='3'>
            <table border='1' cellpadding='0' width='100%' style='border-collapse: collapse; font-family:Century Gothic,Arial; '>
              ...
              <tr>
                <td style=' text-align: right; padding: 10px;'>  Tipo: </td><td style='padding: 10px;'> <b>  ";   $cuerpo .= $nomActividad." </b></td>
              </tr>

              <tr>
                <td style=' text-align: right; padding: 10px;'>  Folio: </td><td style='padding: 10px;'> <b>  ";  $cuerpo .= htmlentities($datos11["IdTarea"])." </b></td>
              </tr>
            </table>
          </td>
          <tr>
